...
use crate::{
    ...
    service::{BytesBody, DefaultHttp2Client, FromBodyStructure},
};
use hyper::body::Body;
...
/// TODO document
#[derive(Debug, Clone)]
pub struct Control<S = DefaultHttp2Client<BytesBody, BytesBody>, ReqB = BytesBody, ResB = BytesBody> {
...
impl Control<DefaultHttp2Client<BytesBody, BytesBody>, BytesBody, BytesBody> {
    pub async fn default_http_clients(
        cmd: &Relentless,
        configs: &Vec<Config>,
    ) -> RelentlessResult<Vec<HashMap<String, DefaultHttp2Client<BytesBody, BytesBody>>>> {
        ...
        for c in configs {
            ...
            for (name, destination) in cmd.override_destination(&c.worker_config.destinations) {
                ...
                let client = DefaultHttp2Client::<BytesBody, BytesBody>::new(authority).await?;
