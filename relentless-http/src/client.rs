...
pub struct HttpClient<ReqB, ResB> {
    client: reqwest::Client, // TODO use hyper ?
    phantom: PhantomData<(ReqB, ResB)>,
}
impl<ReqB, ResB> Clone for HttpClient<ReqB, ResB> {
    fn clone(&self) -> Self {
...
mod tests {
    ...
    async fn test_default_http_client() {
        ...
        let mut client = HttpClient::<Bytes, reqwest::Body>::new().await.unwrap();
